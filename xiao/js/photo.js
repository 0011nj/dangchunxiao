...
function rsort(n){
	...
	for(var s=0;s<_photo.length;s++){
		...
		_photo[s].style.left='';
		_photo[s].style.top='';
		_photo[s].style['transform']=_photo[s].style['-webkit-transform']='rotate(360deg) scale(1.4)';
	...
	photo_center = photos.splice(n,1);//去掉中间的海报
...
//		photo.style['transform']=photo.style['-webkit-transform']='rotate('+random([-150,150])+'deg) scale(1)';
		photo.style["transform"] = "rotate(" + random([-180,180]) + "deg) scale(.8) translate(300px)";//环形排列
	}
...
//		photo.style['transform']=photo.style['-webkit-transform']='rotate('+random([-150,150])+'deg) scale(1)';
		photo.style["transform"] = "rotate(" + random([-180, 180]) + "deg) scale(.8) translate(-300px)";//环形
	}
	//控制按钮处理
	var navs = g('.i');
